...
export async function GET(request: Request) {
  ...
  try {
    ...
    const errorResponse = NextResponse.json({ 
      ...
      details: message.includes('403') ? 'File access denied or blocked' : message 
    }, { status: 500 });
    
    errorResponse.headers.set('Access-Control-Allow-Origin', '*');
    errorResponse.headers.set('Access-Control-Allow-Methods', 'GET, HEAD, OPTIONS');
    errorResponse.headers.set('Access-Control-Allow-Headers', 'Range, Content-Range, Content-Length, Content-Type');
